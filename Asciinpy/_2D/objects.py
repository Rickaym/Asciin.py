...
class Tile(Plane):
    def __init__(self, coordinate: Tuple[int, int], length: Union[Tuple[int, int], int], texture: str= DEFAULT_BRICK, color=None):
...
class Polygon(Mask):
    def __init__(self, coordinates: List[Tuple[int, int]], texture: str=None, color: Color=None):
    ...
    def x(self, value: AnyInt):
        ...
        self.coordinates = list(map(lambda coord: (coord[0]+translates, coord[1]), self.coordinates))
    ...
    def y(self, value: AnyInt):
        ...
        self.coordinates = list(map(lambda coord: (coord[0], coord[1]+translates), self.coordinates))
    ...
    def get_edges(coordinates: List[Tuple[int, int]]) -> Tuple[Line]:
        ...
        for i in range(len(coordinates)):
            if i+1 != len(coordinates):
                edges.append(Line(coordinates[i], coordinates[i+1]))
